...
O ambiente de desenvolvimento (Host) é o **Ubuntu Desktop 24.04**.
  ...
  * **Status do Sandbox (RESOLVIDO):** As permissões do binário `chrome-sandbox` foram corrigidas manualmente (propriedade `root`, modo `4755`). O Electron agora pode executar com o **Sandbox (isolamento) ativado**, garantindo um ambiente de desenvolvimento mais estável.
  * **Comando de Inicialização:** O script `start` em `package.json` **NÃO** requer mais a *flag* `--no-sandbox`. Deve ser mantido o comando padrão e funcional:
    ```json
    "start": "electron ."
    ```
...
1.  Desenvolvimento de módulos Node.js e comunicação IPC (Inter-Process Communication) entre `main.js` e o `renderer`.
